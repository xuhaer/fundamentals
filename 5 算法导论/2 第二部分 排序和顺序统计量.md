...
heapsort(a)
```
上面代码在不在乎原数据 a 的情况下，也可直接先`heapify(a)`, 然后`[heappop(a) for i in range(len(a))]` (a 会为空堆)
...
def partition(A, left, right):
    '''以 x 为主元将 A 划分(原址)为3部分:其中(左部分<=x<右部分), 并返回 x 的index'''
    x = A[right]
    i = left
    for j in range(left, right):
        if A[j] < x:
            A[i], A[j] = A[j], A[i] # 下标 i 左边(从i=lelt开始)的数均比 x 小
            i += 1
    A[i], A[right] = A[right], A[i]
    return i


def quick_sort(A, pivot, right):
    if pivot < right:
        pivot_index = partition(A, pivot, right)
        quick_sort(A, pivot=pivot, right=pivot_index - 1)
        quick_sort(A, pivot=pivot_index + 1, right=right)


a = [1, 3, 4, 2]
quick_sort(a, pivot=0, right=len(a) - 1)
print(a)
```
...
### 基数排序(Radix sort)
基数排序的使用范围仅限于有基数的东西(而许多需要排序的东西没有基数，比如带结构的数据)。
...
即便如此，基数排序是否比基于比较的排序算法(比如快速排序)更好呢？ 通常情况，基数排序的运行时间为 O(n)，这一结果看上去要比快速排序的期望运行时间代价 O(nlgn)更好一些。但是，在这两个表达式中，隐含在 O 符号背后的常数项因子是不同的。在处理 n 个数据时，尽管基数排序执行的循环轮数会比快速排序少，但没一轮其耗费的时间要长得多。哪一个排序算法更合适依赖于具体实现和底层硬件的特性(例如，快速排序通常可以比基数排序更有效地使用硬件的缓存)、输入数据的特征，以及是否额外占用空间等因素。此外，使用计数排序作为中间稳定排序的基数排序不是原址排序，而很多 O(nlgn)的比较排序算法是原址排序。


### 桶排序( Bucket sort)
桶排序假设输入数据服从正态分布，平均情况下它的时间代价为O(n)，与计数排序类似，因为对输入数据作了某种假设，桶排序的速度也很快。具体来说，计数排序建设输入数据都属于一个小区间内的整数，而桶排序则假设输入是由一个随机过程产生，该过程将元素均匀、独立地分布在[0,1)区间上。
...
def insertion_sort(A):
    for i in range(1, len(A)):
        while i > 0 and A[i - 1] > A[i]:
            A[i - 1], A[i] = A[i], A[i - 1]
            i -= 1
    return A # 在 bucket_sort 偷懒了，所以 return 不能少


def bucket_sort(A):
    bucket = [[] for x in range(10)] # 注意：这儿不能[[]] * 10
    res = []
    for el in A:
        bucket[int(el * 10)].append(el)
    for bck in bucket:
        res += insertion_sort(bck)
    return res
